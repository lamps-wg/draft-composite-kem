...
The following 3 examples show the how the KEM Combiner is used to construct the data that is passed to the KDF function to produce the composite KEM `ss`.  See {{sec-kem-combiner}} for additional information.

Each example shows example inputs for each component of the KEM combiner function.  Note that values are shown hex-encoded for display purposes only, they are actually raw binary values.
...
Next, the `Combined KDF Input` to the KDF is given, which is simply the concatenation of the above values.

Finally, the KDF Function and the `ss` Output is given, which was generated by applying the KDF to the `Combined KDF Input`
